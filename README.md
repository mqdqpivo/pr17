...
- Реализовать refresh‑токены и отдельный эндпоинт для обновления JWT.
- Добавить смену пароля и редактирование профиля на странице `/settings`.
- Расширить аудит (логировать больше действий).
- Обернуть проект в Docker‑контейнеры (отдельно для backend, frontend и БД).
